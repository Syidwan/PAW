...
import { createBoard } from "@/lib/actions-org"; // Import server action
...
const CreateBoardForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const result = await createBoard(formData, orgId);
      if (result?.newBoardId) {
        console.log("Redirecting to board:", result.newBoardId);
        router.push(`/board-org/${result.newBoardId}`); // Redirect to the new board
      } else {
